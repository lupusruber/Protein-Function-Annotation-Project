...
def trans_edge_fea_to_sparse(raw_edge_fea, graph, interval: list, is_log=False):
    ...
    graph.update_all(
        ...
        fn.sum("sparse_c", "sparse_f" if is_log else "sparse"),
    )
    if is_log:
        graph.apply_nodes(
            lambda nodes: {"sparse": torch.log2(nodes.data["sparse_f"] + 1)}
        )
...
def add_changed_data(graph, labels, train_idx, test_idx, val_idx, evaluator):
    ...
    # mapper
    mapper = {node: index for index, node in enumerate(sampled_nodes.tolist())}
    ...
    def filter_indices(indices):
        return torch.Tensor(
            [mapper[idx.item()] for idx in indices if idx in sampled_nodes]
    ...
    print(
        f"Nodes : {sampled_graph.number_of_nodes()}\n"
...
#         N_MOST_COMMON = 500
    ...
    pyg_data = Data(
        ...
        edge_attr=pyg_data["protein", "interacts", "protein"]["edge_attr"],
        x=pyg_data["protein"]["x"],
    )
    pyg_data["labels"] = labels
...
def preprocess(
    ...
    if sparse_encoder is not None and len(sparse_encoder) > 0:
        ...
        edge_sparse = trans_edge_fea_to_sparse(
            graph.edata["feat"], graph, simple_inter, is_log
        )
