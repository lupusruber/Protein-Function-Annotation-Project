...
class GIPAWideConv(nn.Module):
    def __init__(
        ...
        self.attn_dst_fc = (
            nn.Linear(self._in_src_feats, n_heads, bias=False) if use_attn_dst else None
        )
        self.attn_edge_fc = (
            nn.Linear(edge_feats, n_heads, bias=False) if edge_feats > 0 else None
        )
    ...
    def agg_func(self, h):
        if self._agg_layer_norm:
            ...
            var = (
                h.var(dim=-1, unbiased=False).view(h.shape[0], self._n_heads, 1) + 1e-9
            )
            h = (h - mean) * self.scale * torch.rsqrt(var) + self.offset
        return self.agg_fc(h.view(-1, self._out_feats * self._n_heads)).view(
    ...
    def forward(self, graph, feat_src, feat_edge=None):
        with graph.local_scope():
            ...
            if self._edge_agg_mode == "single_softmax":
                graph.edata["a"][eids] = edge_softmax(
                    graph, e[eids], eids=eids, norm_by="dst"
                )
            else:
            ...
            # graph normalize
            if self._norm == "adj":
                graph.edata["a"][eids] = graph.edata["a"][eids] * graph.edata[
                    "gcn_norm_adjust"
                ][eids].view(-1, 1, 1)
            if self._norm == "avg":
                graph.edata["a"][eids] = graph.edata["a"][eids] * graph.edata[
    ...
    def forward(self, graph, feat_src, feat_edge=None):
        with graph.local_scope():
            ...
            if self._edge_agg_mode == "both_softmax":
                graph.edata["_a"] = torch.sqrt(
                    edge_softmax(graph, e, norm_by="dst").clamp(min=1e-9)
            ...
            if self._norm == "adj":
                graph.edata["_a"] = graph.edata["_a"] * graph.edata[
                    "gcn_norm_adjust"
                ].view(-1, 1)
            if self._norm == "avg":
                graph.edata["_a"] = (
                    graph.edata["_a"] * graph.edata["gcn_norm"].view(-1, 1)
                ) / 2

            graph.update_all(
                fn.u_mul_e("_feat_src_fc", "_a", "_m"), fn.sum("_m", "_feat_src_fc")
            )
            ...
            if self._use_prop_edge and self._edge_prop_size > 0:
                ...
                if self.attn_dst_fc is not None:
                    ...
                    graph.apply_edges(
                        fn.u_add_v("_attn_src_e", "_attn_dst_e", "_attn_node_e")
                    )
                else:
                ...
                if self._edge_agg_mode == "both_softmax":
                    graph.edata["_a_e"] = torch.sqrt(
                        edge_softmax(graph, e_e, norm_by="dst").clamp(min=1e-9)
                ...
                if self._norm == "adj":
                    graph.edata["_a_e"] = graph.edata["_a_e"] * graph.edata[
                        "gcn_norm_adjust"
                    ].view(-1, 1)
                if self._norm == "avg":
                    graph.edata["_a_e"] = (
                        graph.edata["_a_e"] * graph.edata["gcn_norm"].view(-1, 1)
                    ) / 2

                graph.edata["_m_e"] = graph.edata["_a_e"] * graph.edata["_prop_edge"]
                graph.update_all(
                    fn.copy_e("_m_e", "_m_copy_e"),
                    fn.sum("_m_copy_e", "_feat_src_fc_e"),
                )
...
class GipaWide(nn.Module):
    def __init__(
        ...
        self.first_layer_norm = (
            nn.BatchNorm1d(first_hidden) if first_layer_norm else None
        )
        ...
        self.last_layer_drop = nn.Dropout(
    ...
    def forward(self, g):
        ...
        h = (
            subgraphs[0].srcdata["sparse"]
            if self._use_node_sparse
            else subgraphs[0].srcdata["feat"]
        )
